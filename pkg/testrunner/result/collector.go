...
func New(log logr.Logger, config Config) (*Collector, error) {
	...
	if config.TelemetryControllerBin != "" {
		var err error
		collector.telemetry, err = telemetryctrl.New(logger.Log.WithName("telemetry-controller"), config.TelemetryControllerBin, "1s")
		if err != nil {
...
func (c *Collector) PreRunShoots(kubeconfigPath string, list template.ShootRunList) error {
	...
	if _, err := c.telemetry.StartForShoot(list[0].Parameters.ShootName, list[0].Parameters.Namespace, kubeconfigPath, telemetryOutputDir, c.log.V(3).Enabled()); err != nil {
...
func (c *Collector) PreRunGardener(kubeconfigPath string) error {
	...
	if _, err := c.telemetry.Start(kubeconfigPath, telemetryOutputDir, false); err != nil {
		return errors.Wrap(err, "unable to start telemetry controller")
	}
	return nil
}
